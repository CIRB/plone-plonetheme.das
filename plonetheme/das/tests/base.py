...
class Layer(BasePTCLayer):
    ...
    def afterSetUp(self):
        ...
        from plonetheme import das
        zcml.load_config('configure.zcml', package=das)
        fiveconfigure.debug_mode = False
        # after which the required packages can be initialized
        installPackage('plonetheme.das', quiet=True)
        # finally load the testing profile
        self.addProfile('plonetheme.das:default')
...
<configure xmlns="http://namespaces.zope.org/zope"
           ...
           package="plonetheme.das"
           i18n_domain="test">

    <gs:registerProfile
        name="testing"
        title="plonetheme.das testing"
...
class ThemedasTestCase(Sandboxed, ptc.PloneTestCase):
